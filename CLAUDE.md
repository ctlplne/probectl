...
- **Runtime gating** via `internal/license`: an offline-verifiable **Ed25519-signed license file** (tier, features, expiry, tenant band, customer), verified against build-time-baked public keys — **never phone-home**. The single feature→tier table lives in `internal/license` (`tierFeatures`); gating is wired only at the `main.go` `Build*` seams, never scattered through handlers.
