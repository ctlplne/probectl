...
COVER_PKGS := ./internal/apierror/... ./internal/otel/... ./internal/otel/otlp/... ./internal/version/... \
	...
	./ee/provider/... ./ee/silo/... ./ee/billing/... ./ee/tenantkeys/... ./ee/remediation/... ./internal/branding/... ./internal/cluster/... ./internal/fairness/... ./internal/govern/... ./internal/support/... ./internal/tenantcrypto/... ./internal/tenantlife/... ./internal/remediation/...
...
scale-gate: ## The S48 L/XL/XXL scale gate at FULL scale (reference hardware): make scale-gate TIER=L
	PROBECTL_SCALE=1 PROBECTL_SCALE_TIER=$(or $(TIER),L) \
		$(GO) test -count=1 -v -timeout 120m -run '^TestScaleGate' ./internal/perf/
...
.PHONY: load-test
load-test: ## U-005/SCALE-001 full-stack L/XL/XXL load gate on reference hardware: make load-test TIER=L (commit the result rows to docs/scale-gate.md).
	PROBECTL_TEST_KAFKA=$(or $(PROBECTL_TEST_KAFKA),localhost:9092) \
	PROBECTL_PROM_URL=$(or $(PROBECTL_PROM_URL),http://localhost:9090) \
	PROBECTL_FLOWSTORE_URL=$(or $(PROBECTL_FLOWSTORE_URL),http://probectl:probectl@localhost:8123) \
	PROBECTL_RUN_FULLSTACK_LOAD=1 \
...
scale-fullstack: ## EXC-GATE-01: the full reference-hardware scale gate — in-process scale gate + full-stack load gates at TIER (L|XL|XXL) end to end through real Kafka + Prometheus + ClickHouse. Run on reference HW, then record the result rows in docs/scale-gate.md and promote the PROVISIONAL SLOs. See the "Reference-hardware full run + 72h soak" runbook there.
	...
	PROBECTL_SCALE=1 PROBECTL_SCALE_TIER=$(or $(TIER),L) \
		$(GO) test -count=1 -v -timeout 180m -run '^TestScaleGate' ./internal/perf/
...
interop-offline: ## Hermetic offline protocol-fixture replay gate for OTLP, remote-write, flow, device, and BGP protocols.
...
.PHONY: interop-stock-offline
interop-stock-offline: ## Strict stock-client proof gate: requires pinned stock executable or stock-emitted artifact provenance.
	python3 scripts/check_interop_offline.py SELFTEST
	GO=$(GO) python3 scripts/check_interop_offline.py --manifest test/interop/manifest.json --require-stock-proof --run-replays
...
lint-go: ## gofmt + vet + golangci-lint + crypto-import/editions/no-stringbuilt-sql/unified-TLS guards.
	...
	./scripts/check_layout_map.sh SELFTEST && ./scripts/check_layout_map.sh
	./scripts/check_package_docs.sh SELFTEST && ./scripts/check_package_docs.sh
	...
	./scripts/check_authz_chokepoint.sh SELFTEST && ./scripts/check_authz_chokepoint.sh
	./scripts/check_wire_bounds.sh SELFTEST && ./scripts/check_wire_bounds.sh
