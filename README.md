...
<p align="center">
<a href="https://github.com/ctlplne/probectl/actions/workflows/ci.yml"><img alt="CI" src="https://github.com/ctlplne/probectl/actions/workflows/ci.yml/badge.svg"></a>
...
open first. The implemented capability is evidence consolidation: the incident
view puts cross-plane evidence, owner context, and the next safe human action in
one tenant-scoped place. Reducing **MTTI** (mean time to identify) or **MTTR**
(mean time to resolve) is a design intent, not a measured outcome: the repository
does not yet contain a customer or proof-of-value receipt measuring either one.
...
| **BGP / routing** | RouteViews + RIPE RIS ingestion, direct mTLS BMP sessions from routers, route/path analysis, RPKI validity, a collective internet-outage view |
| **Flow analytics** | NetFlow / sFlow / IPFIX plus AWS VPC, Azure NSG, and GCP VPC flow-log import into ClickHouse, with local learned per-tenant anomaly detection |
| **Device telemetry** | tenant-scoped SNMP discovery/import review, SNMP polling + gNMI streaming, folded into the topology graph |
