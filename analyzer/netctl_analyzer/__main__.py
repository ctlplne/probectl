"""CLI entry point: ``python -m netctl_analyzer --config cfg.json --mrt dump.mrt``.

Emits ``netctl.bgp.events`` as JSON Lines on stdout (or ``--out``), which the Go
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="netctl-analyzer", description="netctl BGP analyzer")
