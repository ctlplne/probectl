...
| `control.env.example`            | template for the optional `control.env` the control service loads when present — any other `PROBECTL_*` key from `docs/configuration.md` (provider bootstrap token, deployment profile, log level, …); never committed |
| `agents.yml`                     | **Agent listener overlay** — enables the gRPC/mTLS agent transport on 9443 with the API's server certificate and the exported agent CA (`agent-ca init` + `agent-ca export` first); without it no producer can attach to the production stack |
| `provider.yml`                   | **MSP / provider-plane overlay** — signed WORM audit export + IR public keyring on the persistent volume, so a licensed MSP evaluation starts on one host; layer after `license.yml`                    |
...
# ghcr.io/ctlplne/probectl-control:v0.6.4@sha256:<release-digest>
