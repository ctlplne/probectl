...
| `PROBECTL_DRAIN_GRACE`             | `5s`                                                               | drain window at shutdown: readiness answers `503` while the listener stays open, so probes and load balancers stop routing before connections are refused; capped at half `PROBECTL_SHUTDOWN_TIMEOUT`; `0` disables (DPR-103) |
...
| `PROBECTL_BUS_CREATE_TOPICS`        | `true`      | the control plane creates the bus topics it publishes and consumes — the shared lanes at startup and a siloed tenant's namespaced lanes when that tenant appears — instead of relying on broker auto-creation (production brokers usually disable it, and before DPR-047 the client never even asked for it, so the first agent's results were refused with "publish not durable"). `false` = you pre-create them (list in [`docs/deploying-agents.md`](deploying-agents.md#bus-topics)); startup then verifies and fails closed naming any missing topic |
| `PROBECTL_BUS_TOPIC_PARTITIONS`     | `3`         | partitions for a topic the plane creates (1–1024; existing topics are left as they are) |
| `PROBECTL_BUS_TOPIC_REPLICATION`    | `-1`        | replication factor for a topic the plane creates; `-1` = the broker's `default.replication.factor` |
...
| `PROBECTL_AUDIT_WORM_DIR` | (none) | enable write-once audit export — the provider audit chain is exported as Ed25519-signed segments into this directory (mount an S3/MinIO **object-lock** bucket for true write-once-read-many) and chain-verified each cycle. Set it through the chart's `audit.worm.*` values, not by hand: `audit.worm.enabled=true` with `audit.worm.existingClaim` pointed at an object-lock volume, and optionally `audit.worm.mountPath`. WORM segments are a durability requirement of their own — they do **not** live in the tenant artifact store, so a deployment can keep artifacts on S3/MinIO and still write signed segments (DPR-116). WORM never renders onto `emptyDir` |
...
| `PROBECTL_AUDIT_RETENTION` | `0` in `single`; `8760h` in `multi-tenant` / `regulated` | deployment maximum for local audit-log retention. `0` keeps provider history indefinitely in the single profile; a tenant may still set a finite `audit_retention_days` tightening. With a positive deployment value, tenant settings above it are rejected and `NULL` inherits it. The hourly runner resolves each tenant's effective window, but tenant rows prune only at or below their durable SIEM cursor; provider rows use the lower of the verified signed-WORM watermark and, when encrypted IR attribution is active, the independently signed IR companion-coverage watermark. `multi-tenant` / `regulated` reject disabled retention and missing `PROBECTL_AUDIT_WORM_DIR`, WORM signing key, or SIEM endpoint so raw audit PII cannot silently keep forever. The runner deletes only a contiguous eligible prefix, appends `audit.retention_prune` receipts carrying the effective window, atomically persists subject-erasure marker hashes before their exported rows age out, and fails closed on unexported, uncovered, tampered, or in-window evidence. `/readyz.audit_retention` and `probectl_audit_retention_*` metrics expose the deployment-level provider posture. Set per the org's SOC2 CC7 / ISO 27001 A.12.4 evidence-retention requirement |
...
| `PROBECTL_IR_PUBLIC_KEY_DIR` | (none) | absolute path to the operator-owned, local-only IR wrapping-key directory. Each tenant has `<tenant-uuid>.pem`, an RSA-3072 public key: mint the pair with `probectl audit ir-keygen <tenant-id>` and install the public half with `probectl-control ir-key-install <tenant-id>` (stdin). The real provider break-glass transaction envelope-seals attribution with that public key and fails closed (`409 ir_key_unavailable`) if it is missing; the routine runtime cannot unseal because no private key is loaded. This is a distinct key domain from `PROBECTL_ENVELOPE_KEY`, makes no network calls, and remains air-gap capable. The same persisted WORM signing key signs the opaque sidecar chain; licensed provider-plane admission therefore also requires `PROBECTL_AUDIT_WORM_DIR` so deleting all stage rows can never look like an unused sidecar. |
...
| `PROBECTL_ALERT_SMTP_ADDR`          | (none)                                                           | SMTP endpoint (`host:port`) for the rule-level **email** channel. Unset keeps email off: rules with an email channel are skipped with a logged warning and the channel-test endpoint reports the failure |
| `PROBECTL_ALERT_SMTP_FROM`          | (none)                                                           | envelope sender address; required with `_ADDR` |
| `PROBECTL_ALERT_SMTP_TLS_MODE`      | `starttls`                                                       | `starttls` (server MUST advertise STARTTLS — a plaintext-only server is refused) \| `implicit` (SMTPS from the first byte, usually :465). There is no plaintext mode (§7 guardrail 12); certificates are always validated |
| `PROBECTL_ALERT_SMTP_USERNAME`      | (none)                                                           | SMTP AUTH username; set together with `_PASSWORD`. Authentication happens only after the session is TLS-secured |
| `PROBECTL_ALERT_SMTP_PASSWORD`      | (none)                                                           | SMTP AUTH password; a secret reference (`secret://…`) is resolved through the secrets backend at boot, and an unresolvable configured credential refuses to start. Never logged |
| `PROBECTL_INCIDENT_WINDOW`          | `10m`                                                            | time window within which related signals correlate into one incident |
| `PROBECTL_PROVISION_REAP_INTERVAL` | `15m`  | how often one elected replica sweeps stranded tenant-provisioning attempts. An attempt is a staging row created before the external silo legs run: it holds no routable tenant and no licensed band slot, and provisioning is now **compensating** (a failed leg drops the databases the earlier legs created), so a stranded row means the attempt was abandoned, not that data is orphaned |
| `PROBECTL_PROVISION_REAP_AFTER`    | `24h`  | how old an attempt's LAST try must be before the sweep abandons it. Abandoning tears the external silo down **first**, then removes the staging row |
| `PROBECTL_PROVISION_REAP_MAX`      | `25`   | how many attempts one sweep may abandon — the sweep is destructive, so it is bounded per run |
| `PROBECTL_TOPOLOGY_WHATIF_MAX_AGENTS` | `2000`                                                         | how many agents one what-if simulation expands routes for. What-if runs a breadth-first search per agent plus two whole-graph reachability passes on a **synchronous** handler, so it is bounded: exceeding a bound returns an **honest partial result** (`partial: true` plus a note naming the bound), never an unbounded run |
| `PROBECTL_TOPOLOGY_WHATIF_MAX_VISITS` | `2000000`                                                      | total nodes one simulation may visit across every traversal — the bound that tracks real cost on a dense graph |
| `PROBECTL_TOPOLOGY_WHATIF_TIMEOUT`    | `5s`                                                           | wall-clock bound on one simulation, applied on both `POST /v1/topology/whatif` and the export GET |
...
| `PROBECTL_CLICKHOUSE_BASIC_AUTH_FILE` | (none) | optional owner-only (`0600`), regular non-symlink JSON credential file shared by pooled ClickHouse-backed planes: `{"username":"...","password":"..."}`. A separate certificate-verifying client pins that credential to each configured store URL's exact origin and path prefix; redirects/routing outside that namespace fail closed, and remote endpoints must use HTTPS. It stays pinned to the pooled store origins; routed silo/residency origins get their own credentials through `PROBECTL_DATAPLANE_BASIC_AUTH_FILES` (DPR-044) |
| `PROBECTL_DATAPLANE_BASIC_AUTH_FILES` | (none) | one owner-only (`0600`) `{"username":"...","password":"..."}` credential file per named data plane — `name=/abs/file[;name=/abs/file...]`, names from `PROBECTL_DATAPLANES`. Each file is pinned to exactly that plane's ClickHouse origin by the same certificate-verifying, origin-bound client the pooled stores use, so a siloed or residency-pinned tenant's database is reached with its plane's credential and never with the pooled one. Every plane must have one (fail closed at startup) unless it is a plaintext loopback development target; a plane that shares the pooled ClickHouse origin must name the same file as `PROBECTL_CLICKHOUSE_BASIC_AUTH_FILE`, which is required whenever this is set. Helm: stage the files through `control.credentialFiles` (DPR-044) |
...
| `PROBECTL_EBPF_L7_HOST_ROOT`   | (none)      | where the NODE's filesystem is mounted inside the agent container, when TLS capture must attach a uprobe to the node's own libssl rather than the container's. Must be an absolute path (the chart mounts `/host`). Discovery looks under it for the library, and refuses a copy that is present but has no execute bit, because the uprobe loader will not attach to one — Debian and Ubuntu package shared libraries `0644`, which is exactly that case (DPR-125). Empty means look only inside the container |
...
| `PROBECTL_EBPF_MAX_BATCH_BYTES` | `786432`  | per-record bound for one published `FlowBatch` (DPR-071). The service map is a cumulative snapshot that travels with every flush, so an unbounded record outgrows the bus's per-message limit (Kafka: 1,000,012 bytes client-side, 1 MiB broker `message.max.bytes`) on a busy host — after which every flush is rejected asynchronously. A flush above the bound is split into several records that together carry exactly the flush. `0` = unbounded (lightweight/test only). Undelivered records are logged at ERROR (`ebpf publish FAILED`, with the bus's reason), counted in `publish_failures_total`, and take the readiness probe down until a flush delivers cleanly |
...
| `PROBECTL_EBPF_HEALTH_STATE_DIR` | (none)    | write `live.json` and `ready.json` health state files for exec probes. The Helm DaemonSet sets this from `health.stateDir` by default so probes work without opening a plaintext listener; each state file is limited to 16 KiB and oversized files fail closed before decoding |
...
| `PROBECTL_INGEST_STRICT_TENANT_LANES` | profile | WIRE-001: refuse agent-published collector planes (flow/eBPF/device/endpoint) on the **shared pooled bus lane**, forcing them onto tenant-namespaced lanes (broker-ACL isolated, forgery-proof). Every active tenant — pooled or siloed — owns the lane `probectl.t-<slug>.<plane>`, created by the control plane and printed by collector registration as `PROBECTL_<PLANE>_BUS_NAMESPACE` (DPR-049). Closes the residual shared-lane forgery surface. Defaults ON under `multi-tenant`/`regulated`, OFF under `single`. Rejections increment `probectl_pipeline_tenant_rejected_total` on `/metrics` |
...
| `PROBECTL_LICENSE_PUBKEYS_B64` | (none) | **build-time**, not runtime: comma-separated base64 Ed25519 public keys linked into `probectl-control` and `probectl-license` as the license trust anchors (`make` variable / Docker `LICENSE_PUBKEYS_B64` build-arg / `scripts/build-release-binaries.sh`). Release builds ship the production anchors; an evaluation build links the evaluation signer instead. A binary built without any anchor cannot verify a license and says so at `version` and `probectl-license verify` (DPR-001); the runtime never reads this variable from the environment. |
...
boundary: Offboard is status-only, while retained-store reclamation requires a
separate tombstone-aware maintenance flow.
...
`privacy.subject_erase` marker and its tenant-routed
`audit_subject_erasures` projection row store only a tenant-scoped subject hash;
later audit reads/exports replace matching structured actor/target/data values
with an erased token while preserving the raw chain links. The projection
and every stable-alias marker commit atomically with all classified Postgres
subject deletions; any alias-marker failure rolls the whole tenant transaction
back. Audit retention also captures marker-only events from rolling old writers
before pruning their exported prefix, so a marker never becomes a permanent
retention barrier and redaction never expires with that event row. Subject
portability exports match this hash directly, and both subject and full-tenant
bundles serialize audit rows through the canonical projection while preserving
tenant, sequence, timestamp, and chain fields. WORM provider exports are
