...
| `PROBECTL_IR_PUBLIC_KEY_DIR` | (none) | absolute path to the operator-owned, local-only IR wrapping-key directory. Each tenant has `<tenant-uuid>.pem`, an RSA-3072 public key. The real provider break-glass transaction envelope-seals attribution with that public key and fails closed if it is missing; the routine runtime cannot unseal because no private key is loaded. This is a distinct key domain from `PROBECTL_ENVELOPE_KEY`, makes no network calls, and remains air-gap capable. The same persisted WORM signing key signs the opaque sidecar chain. |
