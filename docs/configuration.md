...
| `PROBECTL_AUDIT_RETENTION` | `0` (keep forever) | audit-log retention window. `0` keeps audit history indefinitely; a positive duration (e.g. `8760h` for 1 year) makes the pruner eligible to remove events **older than the window AND already durably WORM/SIEM-exported** — fail closed: an un-exported or in-window event is never deleted, so the in-DB hash chain is never gapped (pruned history survives in the signed WORM segments). Set per the org's SOC2 CC7 / ISO 27001 A.12.4 evidence-retention requirement |
...
| `PROBECTL_EBPF_L7_REDACTION`   | `headers`   | how much of a payload may survive capture: `headers` zeroes bodies and secret-bearing header values (`Authorization`, cookies, `api-key`, `token`, `secret`, `credential`, `x-amz-*`) before anything is retained (protocol metadata survives); `length` captures NO payload bytes (traffic shape only, no parsed calls); `full` (consented debugging) disables masking |
