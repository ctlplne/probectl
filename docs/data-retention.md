...
| **Audit entries**, provider audit, break-glass records, and WORM segments | `confidential` to `restricted` because they prove who touched what | Non-repudiation, compliance evidence, incident response, and break-glass accountability | Postgres audit chains plus optional signed WORM object segments | `PROBECTL_AUDIT_RETENTION` defaults to `0` (keep forever) in the `single` profile and to `8760h` (365 days) in `multi-tenant` / `regulated`. Production-like profiles fail closed unless `PROBECTL_AUDIT_WORM_DIR`, a stable WORM signing key, and SIEM export are configured. Tenant rows prune only below the durable `siem_delivery` cursor; provider rows prune only below the signed WORM segment watermark. The pruner deletes only a contiguous eligible prefix, emits an `audit.retention_prune` receipt, and keeps subject-erasure markers so projected audit reads/exports stay redacted. Un-exported evidence is never deleted. `/readyz.audit_retention` and `/metrics` expose whether raw rows are actually aging out. |
...
| `PROBECTL_AUDIT_RETENTION` | `0` in `single`; `8760h` in `multi-tenant` / `regulated` | Deployment | Enables the hourly audit-retention runner; tenant rows require the SIEM cursor, provider rows require the WORM watermark, and prune receipts are appended. Production-like profiles reject `0` and missing WORM/SIEM watermarks. |
