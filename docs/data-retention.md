...
| Device telemetry, endpoint/DEM metrics, and SNMP/gNMI attributes | `pii` for management IPs and endpoint-user identifiers; `confidential` for inventory/config posture | Device health, endpoint experience, and operational inventory correlation | TSDB, device/endpoint stores, topology device nodes, and tenant-scoped API views | Metric samples follow the TSDB retention clock (`PROBECTL_TSDB_MEMORY_RETENTION` in memory mode or the remote TSDB's policy). Endpoint latest-view labels are a derived identity cache pruned by `PROBECTL_DERIVED_IDENTITY_RETENTION_DAYS` with `lifecycle.retention_sweep` receipts. Subject lifecycle removes/export matching endpoint latest-view labels and topology device nodes when the in-process stores are wired, while remote aggregate TSDB copies report their delete-series or retention basis. The device agent's in-process correlation cache prunes stale sysName/interface labels with `PROBECTL_DEVICE_CORRELATION_RETENTION` (default 90 days, `0` disables). Durable inventory/config rows are lifecycle-managed and erased with the tenant or data subject. |
