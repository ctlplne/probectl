...
`PROBECTL_AUDIT_RETENTION` is positive; `0` is the default keep-forever posture.
...
| **Audit entries**, provider audit, break-glass records, and WORM segments | `confidential` to `restricted` because they prove who touched what | Non-repudiation, compliance evidence, incident response, and break-glass accountability | Postgres audit chains plus optional signed WORM object segments | `PROBECTL_AUDIT_RETENTION` defaults to `0` (keep forever). A positive window starts an hourly pruner. Tenant rows prune only below the durable `siem_delivery` cursor; provider rows prune only below the signed WORM segment watermark. The pruner deletes only a contiguous eligible prefix, emits an `audit.retention_prune` receipt, and keeps subject-erasure markers so projected audit reads/exports stay redacted. Un-exported evidence is never deleted. |
...
| `PROBECTL_AUDIT_RETENTION` | `0` = keep forever | Deployment | Enables the hourly audit-retention runner; tenant rows require the SIEM cursor, provider rows require the WORM watermark, and prune receipts are appended. |
