...
## Who owns `web/package-lock.json` (DPR-236, decision of record 2026-09-19)

**npm 10, from the digest-pinned `node:22-bookworm-slim` image — never the npm on a
developer's machine.** Regenerate and check it with the pinned toolchain:

```sh
scripts/web_npm.sh install --package-lock-only   # regenerate
scripts/check_web_lock.sh                        # prove it installs (3s)
```

This is a decision rather than a preference because npm 10 and npm 11 disagree about
lockfile *content* in a way that makes a lock written by one unusable by the other:
...
- npm 11 records every platform-specific optional binary (72 `@esbuild/*` and
  ...
  to install `@esbuild/netbsd-arm64` on a `linux/x64` builder and dies with
  `EBADPLATFORM`.
- npm 10 omits those nodes entirely. npm 11 then refuses the lock with `EUSAGE`,
  `Missing: @esbuild/aix-ppc64@0.21.5 from lock file`, 72 times over.

Every consumer in CI is npm 10: the Dockerfile's digest-pinned `node:22` web stage —
which is why a bad lock takes all eight `build-images` jobs and `image-scan` with it —
plus the three `setup-node` jobs at `node-version: "22"` and `web-rendered-a11y` inside
the Playwright container. The artifact is therefore generated by the tool that consumes
it, exactly as the BPF objects are generated by `scripts/run-ebpf-toolchain.sh` rather
than by whatever clang a laptop has.

Two guards, because the two failure shapes need different instruments:

- `internal/cipolicy` `TestWebLockHasNoNonOptionalPlatformBinaries` — static, no docker,
  runs everywhere, catches the npm 11 shape.
- `scripts/check_web_lock.sh` — `npm ci --dry-run` inside the pinned image, run by the
  `web` job with its own SELFTEST. Whether a lock is installable is npm's own tree
  resolution and cannot be derived from the file; an approximation of it once passed the
  exact lock that broke CI, so the authoritative check is npm itself.

**Never validate a lock against an already-populated `node_modules`.** `npm audit`, the
audit-policy gate and the whole test suite all pass against a stale tree while `npm ci`
cannot install at all — that is precisely how DPR-211 shipped a lock that broke eleven
jobs, and the commit message claimed `npm ci` had been run when it had not.
