...
pass. If you just want to _deploy_ probectl, see [`install.md`](install.md)
instead.
...
| Job                      | Gate                                                                                                                                                                                                                             |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action-pins`            | every workflow action is commit-SHA-pinned + supply-pin checks + the self-tested workflow-permission boundary (a job that executes PR code cannot also hold write authority)                                                     |
...
| `packaging-smoke`        | package/install smoke checks stay wired                                                                                                                                                                                          |
...
| `coverage-comment`       | best-effort PR comment from a bounded coverage-summary artifact; this write-capable job depends on `coverage`, performs no checkout, and executes no repository code                                                             |
...
| `openapi-gate`           | specs are valid OpenAPI 3.1, registered core `/v1` plus provider `/provider/v1` routes exactly match them, and every operation has a named CLI path or governed none-by-design exception                                         |
...
| `delivery-audit-gate`    | signed exact-source receipt semantics, out-of-band signer trust, repository-only auditor packaging boundary, and planted fixture-only rejection                                                                                |
...
- **Coverage gate** (`make cover-gate` → the `coverage` CI job) — **statement
  ...
  **floor** fails CI when a package drops below its number. This gate floors the
  ...
  _correctness_ by the `integration` and `cross-tenant-isolation` jobs instead — a
