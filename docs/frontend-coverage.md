...
use it — normally a first-party screen, or an API/CLI when no visual workflow is
needed. The frontend-coverage gate is a
...
| ------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `native`      | A first-class screen in the app.                                                         | The route renders a _real_ screen (not the "coming soon" placeholder), has a `<main>` landmark (the HTML element assistive technology uses to jump straight to the page's content) and an `<h1>`, passes the jsdom `axe` screen test, and passes the rendered Chromium gate in both shipped themes at desktop and mobile widths for browser-computed contrast, keyboard focus visibility/order safety, focus-obscured, and minimum interactive target size. Native entries may also declare API/CLI evidence for PRD row parity. |
| `federated`   | Served through an interoperability surface by design — Prometheus-compatible API, OTLP, raw API, or CLI. External clients remain optional. | The declared evidence actually exists: a `file:<repo path>` is present on disk, an `openapi:<path>` is a real route in the control plane's OpenAPI spec (`internal/control/openapi.json`), or a `cli:<probectl command>` is registered in the terminal surface (`internal/cli/surfaces.go`). Federated surfaces count only where an API/CLI workflow is the product contract; they never substitute for a native dashboard or other visual workflow. |
...
- PRD rows whose delivered capability no longer traces to native, API, CLI,
  federated, or explicit `none-by-design` evidence;
