...
| `hp-topology-rebuild` | Topology cold-start replay/rebuild | topology observations -> `IndexedStore` | 500 ms | 2 s | 10 s | 3,200 observations/s | CI logs three raw S/M/L replay/snapshot/total samples, gates their median, and requires every sample complete; strict reference-hardware benchmark receipts cover the performance claim and XL/XXL fixtures |
...
- `go test ./internal/perf -run TestTopologyRebuildTargets -count=1 -v`
  replays each S/M/L tier three times into a fresh store, logs every raw
  replay/snapshot/total sample plus the median aggregation rule, and fails on a
  sustained timing regression. Every sample must still pass tenant completeness
  and ghost-tenant correctness. The CI timing verdict is a scheduling-noise
  resistant regression guard; strict benchmark receipts on reference hardware
  remain the authority for buyer-facing performance claims.
