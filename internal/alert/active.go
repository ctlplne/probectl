...
//
// Only firing/pending evaluation state is volatile: it lives in the in-memory,
// per-tenant evaluator and is re-derived on the first tick after restart.
// Silences/acks are NOT memory-only. The control handlers write them through
// store.AlertOps, and evaluator startup seeds them through RestoreOps, so those
// operator decisions survive process and evaluator restarts.
