...
func fixtureRepo(t *testing.T) string {
	...
	files := map[string]string{
		...
		"internal/cli/cli.go":                          "package cli\nfunc RunWithStdin(args []string) int { cfg := Config{ BaseURL: envOr(getenv, \"PROBECTL_API_URL\", \"https://localhost:8443\"), Token: getenv(\"PROBECTL_API_TOKEN\"), Tenant: getenv(\"PROBECTL_TENANT\"), Locale: i18n.Resolve(getenv(\"PROBECTL_LOCALE\")), SessionCookieFile: getenv(\"PROBECTL_SESSION_COOKIE_FILE\") }; args, cfg.JSON = extractBoolFlag(args, \"--json\"); fs := flag.NewFlagSet(\"probectl\", flag.ContinueOnError); fs.SetOutput(stderr); fs.Usage = func() { usage(stderr, cfg.Locale) }; fs.StringVar(&cfg.BaseURL, \"url\", cfg.BaseURL, \"control-plane API base URL (env PROBECTL_API_URL)\"); fs.StringVar(&cfg.Token, \"token\", cfg.Token, \"API auth token, sent as Bearer (env PROBECTL_API_TOKEN)\"); fs.StringVar(&cfg.Tenant, \"tenant\", cfg.Tenant, \"tenant UUID, sent as X-Probectl-Tenant (env PROBECTL_TENANT)\"); if err := fs.Parse(args); err != nil { return 2 }; rest := fs.Args(); if len(rest) == 0 { return 2 }; switch rest[0] { case \"test\": return cmdTest(cfg, rest[1:], stdout, stderr); case \"agent\": return cmdAgent(cfg, rest[1:], stdout, stderr); case \"lifecycle\": return cmdLifecycle(cfg, rest[1:], stdout, stderr); default: if spec, ok := surfaceCommands[rest[0]]; ok { return cmdSurface(cfg, spec, rest[1:], stdout, stderr) }; return 2 } }\n",
		"internal/cli/generic.go":                      "package cli\nfunc cmdSurface(cfg int, spec struct{ Ops map[string]int }, args []string, stdout, stderr int) int { if len(args) == 0 || args[0] == \"help\" { return 2 }; op, ok := spec.Ops[args[0]]; if !ok { return 2 }; return runRawOperation(cfg, op, args[1:], stdout, stderr) }\nfunc runRawOperation(cfg, op int, args []string, stdout, stderr int) int { path := op.Path; if op.ArgName != \"\" { if len(args) == 0 { fmt.Fprintf(stderr, \"%s: missing <%s>\\n\", op.Path, op.ArgName); return 2 }; path = strings.ReplaceAll(path, \"{\"+op.ArgName+\"}\", url.PathEscape(args[0])); args = args[1:] }; fs := flag.NewFlagSet(op.Method+\" \"+op.Path, flag.ContinueOnError); fs.SetOutput(stderr); bodyRaw := fs.String(\"body\", \"\", \"JSON request body\"); query := queryFlag{}; fs.Var(&query, \"query\", \"query parameter k=v (repeatable)\"); if err := fs.Parse(args); err != nil { return 2 }; if len(fs.Args()) > 0 { fmt.Fprintf(stderr, \"unexpected args: %s\\n\", strings.Join(fs.Args(), \" \")); return 2 }; path = withQueryValues(path, query); body, err := parseBody(*bodyRaw); if err != nil { fmt.Fprintln(stderr, \"invalid --body: \"+err.Error()); return 2 }; var out any; if err := newClient(cfg).do(op.Method, path, body, &out); err != nil { return fail(stderr, err) }; return printGeneric(stdout, out, cfg.JSON, op.Method) }\n",
