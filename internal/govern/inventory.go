...
var dataInventory = []DataInventoryEntry{
	{
		...
		ExportBehavior: "tenant-scoped lifecycle/support exports include result evidence; subject export includes matching TSDB metric names/labels in tsdb_metrics.jsonl when the backend supports it; redacted mode masks target, DNS, and attribute values",
		TenantDelete:   "tenantlife calls TSDB DeleteTenant or records the Prometheus admin-delete manual receipt",
		SubjectDelete:  "memory TSDB DeleteSubject removes tenant-scoped metric samples whose names or labels match; remote TSDB backends require delete_series/federation or retention age-out and are recorded as not_capable",
	},
	{
		ID:             "flow-telemetry",
	...
	},
	{
		...
		ExportBehavior: "tenant support/export bundles include only tenant-scoped aggregates and honor governed redaction; subject export streams matching workload aggregates where the backend supports subject filtering",
		TenantDelete:   "DeleteTenant removes every tenant aggregate and verifies no tenant rows remain",
		SubjectDelete:  "DeleteSubject removes tenant-scoped workload aggregates whose agent/src/dst/L7 labels match when the backend supports it; other aggregate backends age out by eBPF retention and are reported honestly",
	},
	{
		ID:             "path-topology",
		...
		ExportBehavior: "tenant exports include path/topology evidence only after tenant scope; subject export streams matching topology/device graph labels where the backend supports it; redacted mode masks hop IPs and labels",
		TenantDelete:   "tenantlife calls pathstore.DeleteTenant and topology.DeleteTenant",
		SubjectDelete:  "topology DeleteSubject removes matching derived graph labels and connected edges, with device-node counts reported separately; raw path rows still age/delete through pathstore lifecycle",
	},
	{
		ID:             "otlp-traces-logs",
	...
	},
	{
		...
		Store:          "Device telemetry, LLDP/CDP neighbor evidence, endpoint/DEM metrics, SNMP/gNMI attributes, and inventory state",
		...
		DataClasses:    []Class{ClassPII, ClassConfidential, ClassInternal},
		Retention:      "metric samples follow TSDB retention; LLDP/CDP current snapshots prune after 24h; endpoint latest views follow derived identity retention; device-agent correlation labels follow PROBECTL_DEVICE_CORRELATION_RETENTION; durable inventory rows live while the tenant/identity is active",
		RetentionOwner: "internal/store/tsdb, internal/store/deviceneighbors, internal/endpoint, device-agent correlation cache, and Postgres tenant tables",
		Processors: []string{
			"SNMP/gNMI/syslog/trap normalization",
			"bounded LLDP/CDP normalization and tenant-scoped snapshot replacement",
			"tenant-scoped inventory upserts",
			"govern column/category redaction during export",
		},
		ExportBehavior: "tenant lifecycle export includes tenant-scoped inventory and endpoint state; subject export streams matching endpoint latest-view labels and topology device nodes where available; redacted mode masks management IPs and subject fields",
		TenantDelete:   "tenantlife deletes tenant Postgres rows and TSDB series or records manual TSDB deletion requirements",
		SubjectDelete:  "subject lifecycle removes matching identity rows, endpoint latest-view labels, TSDB metric labels, and topology device nodes when the backend supports subject filtering; aggregate-only copies report the retention clock",
	},
	{
