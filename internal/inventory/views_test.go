...
func TestMemoryViewStoreTenantIsolation(t *testing.T) {
	...
	a, err := store.Save(context.Background(), "tenant-a", SaveViewInput{
	...
	if _, err := store.Save(context.Background(), "tenant-b", SaveViewInput{
		Surface: SurfaceEndpoints,
		Name:    "Tenant B",
	...
	rows, err := store.List(context.Background(), "tenant-a", SurfaceEndpoints)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.Get(context.Background(), "tenant-b", a.ID); !errors.Is(err, ErrNotFound) {
...
func TestMemoryViewStoreValidation(t *testing.T) {
	...
	for name, input := range map[string]SaveViewInput{
		...
		if _, err := store.Save(context.Background(), "tenant-a", input); err == nil {
