...
func (c *ClickHouse) EnsureReaderRowPolicy(ctx context.Context, readerUser string) error {
	...
	for _, table := range []string{sharedFlowsTable, sharedFlowRollupsTable} {
		// OR REPLACE is intentional: a fixed policy name with IF NOT EXISTS
		// retains the old TO user after credential rotation, leaving the new
		// reader without a database filter.
...
func topSQL(q TopQuery, table string) (string, chParams) {
	...
	sql := fmt.Sprintf(
		`SELECT %s AS k, %s AS d, sum(bytes_scaled) AS b, sum(packets_scaled) AS p, count() AS f, uniqExactIf(trimBoth(exporter), notEmpty(trimBoth(exporter))) AS e `+
...
func topSeriesSQL(q TopQuery, table string, top []TopRow) (string, chParams) {
	...
	sql := fmt.Sprintf(
		`SELECT toStartOfInterval(ts, INTERVAL %d second) AS t, %s AS k, %s AS d, `+
			`sum(bytes_scaled) AS b, sum(packets_scaled) AS p, count() AS f, uniqExactIf(trimBoth(exporter), notEmpty(trimBoth(exporter))) AS e `+
