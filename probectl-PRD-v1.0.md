...
2. **Multi-tenant by construction** — `tenant_id` as the outermost scope on every record/agent/query/metric/event/object; pooled isolation enforced at the storage layer (Postgres RLS + ClickHouse scoping), siloed/hybrid models, a separately-privileged provider plane with audited break-glass, per-tenant metering/keys/erasure — all CI-gated.
...
- ✅ **Provider/MSP plane.** The consumption-priced MSP tier self-hosts and resells under the probectl banner: tenant lifecycle (provision/suspend/offboard), fleet-across-tenants, and **no implicit telemetry access** — break-glass is explicit, time-bounded, tenant-consented, separately audited. `ee/provider`, `internal/tenantlife`, `docs/provider-plane.md`.
- ✅ **Metering/billing export and per-tenant keys.** MSP usage meters are exported only when an operator requests CSV/JSONL — never phone-home — and may feed the MSP's own customer pricing (`ee/billing`, `internal/usage`); per-tenant envelope keys/BYOK use the crypto seam (`ee/tenantkeys`, `internal/tenantcrypto`, `docs/byok.md`).
...
The repo's claims are enforced by **62 workflow jobs** across `.github/workflows/` (43 in the main CI workflow plus 19 nightly/release/security jobs); the notable standing gates:
...
| F8/F9 | Alerting / dashboards + incident timeline | ✅ | `internal/alert`, `internal/incident`, `web/`; dense Operator/Executive dashboards plus tenant-safe saved views and scheduled local-inbox PDF/CSV reporting use `migrations/0061_dashboard_reporting.sql`, `internal/store/dashboardreports.go`, `internal/control/dashboardreporting.go`; silences/acks persist through `migrations/0043_alert_ops.sql`, `internal/store/alertops.go`, `internal/control/alertsactive.go`, `internal/control/alerteval.go` |
...
OTLP point-type conformance is delivered: the three-signal OTLP path is delivered
for metrics/traces/logs ingest/export and rejects unsupported summary,
exponential-histogram, and unspecified metric data at the authenticated receiver
