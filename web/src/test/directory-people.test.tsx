...
describe('people & roles', () => {
  test('lists people with their roles and adds a teammate with a role', async () => {
    ...
    renderApp('/admin', { me: { permissions: ['directory.read', 'directory.write', 'audit.read'] } })
    ...
    const form = screen.getByRole('form', { name: 'Add a teammate' })
    ...
    await userEvent.click(within(form).getByRole('button', { name: 'Add teammate' }))
  ...
  test('grants and revokes a role, and refuses to remove the last administrator', async () => {
    ...
    renderApp('/admin', { me: { permissions: ['directory.read', 'directory.write', 'audit.read'] } })
    const table = await screen.findByRole('table', { name: 'People & roles' })
    await userEvent.selectOptions(within(table).getByLabelText('Role for viewer@probectl.test'), 'editor')
    await userEvent.click(within(table).getByRole('button', { name: 'Grant role to viewer@probectl.test' }))
    await waitFor(() => expect(within(table).getByRole('button', { name: 'Remove editor from viewer@probectl.test' })).toBeInTheDocument())
    await userEvent.click(within(table).getByRole('button', { name: 'Remove editor from viewer@probectl.test' }))
    await waitFor(() => expect(within(table).queryByRole('button', { name: 'Remove editor from viewer@probectl.test' })).toBeNull())
    await userEvent.click(within(table).getByRole('button', { name: 'Remove admin from operator@probectl.test' }))
